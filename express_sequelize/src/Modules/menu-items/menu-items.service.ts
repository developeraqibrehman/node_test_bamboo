...
export class MenuItemsService {
  ...
  */
  ...
  async getMenuItems() {
    const getMenuItemsRecursive = async (menuItem: any) => {
        ...
        if (children.length > 0) {
            const promises: any = [];
            children.forEach((menuItem:any) => {
                promises.push(getMenuItemsRecursive(menuItem));
            });
            menuItem['children'] = await Promise.all(promises);
        }
        else menuItem['children'] = []; 
        return menuItem;
      };
     const res = await getMenuItemsRecursive({id: null});
     return res?.children?.[0] ? [res?.children?.[0]]: [];
  }
}
